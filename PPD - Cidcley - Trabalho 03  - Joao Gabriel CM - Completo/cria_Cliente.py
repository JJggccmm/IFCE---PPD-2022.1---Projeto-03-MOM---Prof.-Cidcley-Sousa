...
# As estruturas "Queue" são melhores para se guardar as strings de msgs lidas nos tóicos poi são melhores para estruturas com threads (Queues são 'thread safe')!
mensagens_recebidas = Queue()
lista_Topicos_Sensores = Queue()

# Guardaca a string armazenada na Queue 'lista_Topicos_Sensores' separando os topicos na string (pelo método '.split()') em elementos separados na lista abaixo
lista_Topicos = []
...
def fecha_janela_TOPLEVEL_2(client_subscrb,Toplevel):
    client_subscrb.loop_stop()
    Toplevel.destroy()
...
def janela_Cria_Cliente():
    ...
    #threading.Thread(target=loop_checa_Topicos).start() #<---- THREAD 'loop_checa_Topicos' *****
    newWindow.after(1,lambda:loop_checa_Topicos(newWindow))

def janela_Config_Cliente():
    global lista_Topicos_Sensores,lista_Topicos
...
def janela_Principal_Cliente(newWindow_close,nome_Cliente,topicos):
    ...
    newWindow.protocol("WM_DELETE_WINDOW", lambda:fecha_janela_TOPLEVEL_2(client_subscriber_Cliente,newWindow))
    ...
    newWindow.after(1,lambda:mostra_Dados_Recebidos_do_Topico_na_Janela_Principal_Cliente(newWindow,text_area_Sensores))

def mostra_Dados_Recebidos_do_Topico_na_Janela_Principal_Cliente(newWindow_input,text_area):
    global mensagens_recebidas
    ...
    newWindow_input.after(1,lambda:mostra_Dados_Recebidos_do_Topico_na_Janela_Principal_Cliente(newWindow_input,text_area))

def loop_checa_Topicos(newWindow_input):
    global lista_Topicos_Sensores,lista_Topicos
    ...
    if lista_Topicos_Sensores.empty() == True: # Link que ajudou nesse trecho: https://www.w3resource.com/python-exercises/data-structures-and-algorithms/python-data-structure-exercise-28.php
        #print(str(lista_Topicos)+" tá dando como vazia!")
        ...
        if len(lista_Topicos) == 0:
            ...
            s = lista_Topicos_Sensores.get()
            #lista_Topicos.clear() # Limpamos a lista para depois recolocarmos os elementos de novo na mesma ordem, só que atualizados!
            s_temp = s.split(',')
            lista_Topicos.extend(s_temp)
            lista_Topicos = list(dict.fromkeys(lista_Topicos)) # Link que ajudou nesse trecho: https://stackoverflow.com/questions/8177079/take-the-content-of-a-list-and-append-it-to-another-list
        #print(lista_Topicos)

    newWindow_input.after(1,lambda:loop_checa_Topicos(newWindow_input))
...
if __name__ == "__main__":
    janela_Cria_Cliente()
    #threading.Thread(target=janela_Cria_Cliente).start() #<---- THREAD 'janela_Cria_Cliente' *****
    root.mainloop()
